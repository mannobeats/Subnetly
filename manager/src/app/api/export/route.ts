...
export async function GET() {
  try {
    ...
    const [site, devices, subnets, vlans, services, wifi] = await Promise.all([
      ...
      prisma.subnet.findMany({ where: { siteId }, include: { vlan: true, ipAddresses: true, ipRanges: true }, orderBy: { prefix: 'asc' } }),
      prisma.vLAN.findMany({ where: { siteId }, include: { subnets: true }, orderBy: { vid: 'asc' } }),
      prisma.service.findMany({ where: { siteId }, include: { device: true }, orderBy: { name: 'asc' } }),
      prisma.wifiNetwork.findMany({ where: { siteId }, include: { vlan: true, subnet: true }, orderBy: { ssid: 'asc' } }),
    ])
    ...
    md += `---\n\n*This document was auto-generated by Homelab Manager.*\n`
