...
import DeviceFormDialog, {
  type DeviceFormData,
  type SubnetOption,
} from "@/components/devices/DeviceFormDialog";
...
import type { CustomCategory, Device, Site } from "@/types";
...
export default function Home() {
  ...
  const [serviceEnvironments, setServiceEnvironments] = useState<
    CustomCategory[]
  >([]);
  const [serviceHealthStatuses, setServiceHealthStatuses] = useState<
    CustomCategory[]
  >([]);
  ...
  const [selectedServiceFilter, setSelectedServiceFilter] = useState<
    string | null
  >(null);
  const [selectedChangelogFilter, setSelectedChangelogFilter] = useState<
    string | null
  >(null);
  ...
  useEffect(() => {
    const onStorage = () => {
      try {
        ...
        if (saved) {
          const s = JSON.parse(saved);
          if (s.confirmDeletes !== undefined)
            setConfirmDeletes(s.confirmDeletes);
          if (s.autoRefresh !== undefined) setAutoRefresh(s.autoRefresh);
        }
      } catch {
        /* ignore */
  ...
  const computeAvailableIps = (subnetId: string, autoSelect = false) => {
    ...
    devices.forEach((d) => {
      if (d.ipAddress) usedSet.add(d.ipAddress.trim());
    });
    ...
    <div className="flex h-screen w-screen overflow-hidden">
      ...
      <div className="flex-1 flex flex-col overflow-hidden bg-(--bg)">
        ...
        {activeView === "dashboard" && (
          <div
            className="flex-1 overflow-auto p-4 px-6"
            key={`dash-${siteKey}`}
          >
        ...
        {activeView === "ipam" && (
          ...
          >
            <IPPlannerView
              searchTerm={searchTerm}
              selectedIpFilter={selectedIpFilter}
              onPlatformOptionsChange={fetchSitesAndCategories}
            />
          </div>
        )}
        {activeView === "vlans" && (
          ...
          >
            <VLANView
              searchTerm={searchTerm}
              selectedRole={selectedVlanRole}
              vlanRoles={vlanRoles}
              vlanStatuses={vlanStatuses}
              highlightId={highlightId}
            />
          </div>
        )}
        {activeView === "wifi" && (
          ...
          >
            <WiFiView
              searchTerm={searchTerm}
              selectedSecurityFilter={selectedServiceFilter}
              securityOptions={wifiSecurities}
              highlightId={highlightId}
            />
          </div>
        )}
        {activeView === "topology" && (
          <div
            ...
            key={`topo-${siteKey}`}
          >
            <TopologyView selectedCategory={selectedCategory} />
          </div>
        )}
        {activeView === "services" && (
          <div className="flex-1 overflow-auto p-4 px-6" key={`svc-${siteKey}`}>
            <ServicesView
              searchTerm={searchTerm}
              selectedProtocol={selectedServiceFilter}
              protocolOptions={serviceProtocols}
              environmentOptions={serviceEnvironments}
              healthStatusOptions={serviceHealthStatuses}
              highlightId={highlightId}
            />
          </div>
        )}
        {activeView === "changelog" && (
          <div className="flex-1 overflow-auto p-4 px-6" key={`log-${siteKey}`}>
            <ChangelogView
              searchTerm={searchTerm}
              selectedFilter={selectedChangelogFilter}
            />
          </div>
        )}
        {activeView === "settings" && (
          <div className="flex-1 overflow-auto p-4 px-6">
            <SettingsView
              ...
              onCategoriesChange={fetchSitesAndCategories}
              sites={sites}
              activeSiteId={activeSiteId}
              onSitesChange={fetchSitesAndCategories}
            />
          </div>
        )}
      </div>
