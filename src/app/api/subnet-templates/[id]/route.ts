...
export async function PATCH(
  ...
  try {
    ...
    const existing = await prisma.subnetTemplate.findFirst({
      where: { id, siteId },
    });
    if (!existing)
    ...
    if (body.name !== undefined) {
      ...
      const duplicate = await prisma.subnetTemplate.findFirst({
    ...
    const updated = await prisma.subnetTemplate.update({
      ...
      data: {
        ...
        slug,
        prefix:
          body.prefix !== undefined ? String(body.prefix).trim() : undefined,
    ...
    return NextResponse.json(
      { error: "Failed to update subnet template" },
      { status: 500 },
    );
  }
}
...
export async function DELETE(
  ...
  try {
    ...
    return NextResponse.json(
      { error: "Failed to delete subnet template" },
      { status: 500 },
    );
  }
}
