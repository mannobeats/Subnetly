...
export async function PATCH(request: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const subnetExisting = await prisma.subnet.findFirst({ where: { id, siteId } })
    if (!subnetExisting) {
    ...
    await prisma.changeLog.create({
      data: { objectType: 'Subnet', objectId: subnet.id, action: 'update', changes: JSON.stringify(body), siteId },
    })
  ...
  try {
    ...
    if (ipsInSubnet.length > 0) {
      await prisma.device.updateMany({
        where: { siteId, ipAddress: { in: ipsInSubnet.map(ip => ip.address) } },
        data: { ipAddress: '' },
      })
    }
    ...
    await prisma.changeLog.create({
      data: { objectType: 'Subnet', objectId: id, action: 'delete', changes: '{}', siteId },
    })
