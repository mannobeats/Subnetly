  ...
  try {
    ...
    const networks = await prisma.wifiNetwork.findMany({
      ...
      include: {
        ...
        subnet: { select: { id: true, prefix: true, mask: true, description: true, gateway: true } },
      },
      orderBy: { ssid: 'asc' },
    })
    return NextResponse.json(networks)
  } catch {
    return NextResponse.json({ error: 'Failed to fetch WiFi networks' }, { status: 500 })
  }
}
...
export async function POST(request: Request) {
  try {
    ...
    await prisma.changeLog.create({
      data: { objectType: 'WifiNetwork', objectId: network.id, action: 'create', changes: JSON.stringify(body), siteId },
    })
