  ...
  try {
    ...
    if (!backup.version || !backup.site) {
      return NextResponse.json(
        { error: "Invalid backup file format. Missing version or site data." },
        { status: 400 },
      );
    }
    ...
    await prisma.iPRangeSchemeEntry.deleteMany({
      where: { scheme: { siteId } },
    });
    ...
    if (backup.subnets?.length) {
      for (const s of backup.subnets as Record<string, unknown>[]) {
        const vlanId = s._vlanExportId
          ? vlanIdMap[s._vlanExportId as string] || null
          : null;
    ...
    if (backup.ipAddresses?.length) {
      for (const ip of backup.ipAddresses as Record<string, unknown>[]) {
        const subnetId = ip._subnetExportId
          ? subnetIdMap[ip._subnetExportId as string] || null
          : null;
    ...
    if (backup.subnetTemplates?.length) {
      await prisma.subnetTemplate.createMany({
        data: (backup.subnetTemplates as Record<string, unknown>[]).map(
          (t) => ({
            siteId,
    ...
    if (backup.ipRanges?.length) {
      for (const r of backup.ipRanges as Record<string, unknown>[]) {
        const subnetId = r._subnetExportId
          ? subnetIdMap[r._subnetExportId as string] || null
          : null;
        if (!subnetId) continue;
        await prisma.iPRange.create({
          data: {
            ...
            schemeEntryId: r._schemeEntryExportId
              ? schemeEntryIdMap[r._schemeEntryExportId as string] || null
              : null,
            subnetId,
          },
        });
    ...
    if (backup.services?.length) {
      for (const s of backup.services as Record<string, unknown>[]) {
        const deviceId = s._deviceExportId
          ? deviceIdMap[s._deviceExportId as string] || null
          : null;
    ...
    if (backup.wifiNetworks?.length) {
      for (const w of backup.wifiNetworks as Record<string, unknown>[]) {
        const vlanId = w._vlanExportId
          ? vlanIdMap[w._vlanExportId as string] || null
          : null;
        const subnetId = w._subnetExportId
          ? subnetIdMap[w._subnetExportId as string] || null
          : null;
    ...
    return NextResponse.json(
      { error: `Failed to import backup: ${message}` },
      { status: 500 },
    );
  }
}
