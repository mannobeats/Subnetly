...
export async function PATCH(request: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    await prisma.changeLog.create({
      data: { objectType: 'IPAddress', objectId: id, action: 'update', changes: JSON.stringify(body), siteId },
    })
...
export async function DELETE(_request: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    const { siteId } = await requireActiveSiteContext()
    const { id } = await params
    const ip = await prisma.iPAddress.findFirst({ where: { id, subnet: { siteId } } })
    ...
    await prisma.changeLog.create({
      data: { objectType: 'IPAddress', objectId: id, action: 'delete', changes: JSON.stringify({ address: ip.address, dnsName: ip.dnsName }), siteId },
    })
