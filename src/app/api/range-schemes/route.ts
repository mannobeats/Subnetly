...
export async function GET() {
  try {
    ...
    return NextResponse.json({ error: 'Failed to fetch range schemes' }, { status: 500 })
  }
}
...
export async function POST(request: Request) {
  try {
    ...
    const entriesInput = Array.isArray(body.entries) ? body.entries as EntryInput[] : []
    ...
    const duplicate = await prisma.iPRangeScheme.findUnique({ where: { siteId_slug: { siteId, slug } } })
    if (duplicate) return NextResponse.json({ error: 'Scheme already exists' }, { status: 409 })
    ...
    await prisma.changeLog.create({
      data: {
        ...
        objectId: scheme.id,
        action: 'create',
        changes: JSON.stringify({ name: scheme.name, entries: scheme.entries.length }),
      },
    })

    return NextResponse.json(scheme)
  } catch {
    return NextResponse.json({ error: 'Failed to create range scheme' }, { status: 500 })
  }
}
