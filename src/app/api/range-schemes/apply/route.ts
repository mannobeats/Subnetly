...
export async function POST(request: Request) {
  try {
    ...
    if (!subnetId || !schemeId) {
      return NextResponse.json({ error: 'subnetId and schemeId are required' }, { status: 400 })
    }
    ...
    await prisma.changeLog.create({
      data: {
        ...
        objectId: scheme.id,
        action: 'create',
        changes: JSON.stringify({ type: 'apply', subnetId, replaceExisting, entries: scheme.entries.length }),
      },
    })

    return NextResponse.json({ success: true, applied: scheme.entries.length })
  } catch {
    return NextResponse.json({ error: 'Failed to apply range scheme' }, { status: 500 })
  }
}
