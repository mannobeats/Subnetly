...
export async function PATCH(request: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const existing = await prisma.iPRangeScheme.findFirst({ where: { id, siteId }, include: { entries: true } })
    if (!existing) return NextResponse.json({ error: 'Scheme not found' }, { status: 404 })

    const hasEntriesUpdate = Array.isArray(body.entries)
    const entriesInput = hasEntriesUpdate ? body.entries as EntryInput[] : null

    if (entriesInput && entriesInput.length === 0) {
      return NextResponse.json({ error: 'At least one scheme entry is required' }, { status: 400 })
    }
    ...
    if (normalizedEntries) {
      for (const entry of normalizedEntries) {
        if (entry.startOctet > entry.endOctet) {
          return NextResponse.json({ error: 'Each entry start octet must be <= end octet' }, { status: 400 })
        }
      }
    }
    ...
    if (body.name !== undefined) {
      ...
      const duplicate = await prisma.iPRangeScheme.findFirst({ where: { siteId, slug, NOT: { id } } })
      if (duplicate) return NextResponse.json({ error: 'Scheme name already in use' }, { status: 409 })
    }
    ...
    return NextResponse.json({ error: 'Failed to update range scheme' }, { status: 500 })
  }
}
  ...
  try {
    ...
    await prisma.$transaction(async (tx) => {
      if (entryIds.length > 0) {
        await tx.iPRange.updateMany({ where: { schemeEntryId: { in: entryIds } }, data: { schemeEntryId: null } })
      }
    ...
    return NextResponse.json({ error: 'Failed to delete range scheme' }, { status: 500 })
  }
}
