...
export async function GET() {
  try {
    ...
    const subnetStats = subnets.map((s) => {
      ...
      devices.forEach((d) => {
        try {
          ...
          if ((devIp & maskBits) === (prefixNum & maskBits))
            ipamIps.add(d.ipAddress);
        } catch {
          /* skip invalid */
        }
      });
    ...
    const categoryBreakdown = devices.reduce(
    ...
    const healthBreakdown = services.reduce(
      (acc: Record<string, number>, s) => {
        const status =
          ((s as Record<string, unknown>).healthStatus as string) || "unknown";
        acc[status] = (acc[status] || 0) + 1;
        return acc;
      },
      {},
    );
    const monitoredCount = services.filter(
      (s) => (s as Record<string, unknown>).healthCheckEnabled,
    ).length;
    ...
    return NextResponse.json({
      ...
      wifiNetworks: wifiNetworks.map((w) => ({
        ...
        subnet: w.subnet
          ? { prefix: w.subnet.prefix, mask: w.subnet.mask }
          : null,
      })),
    });
  } catch {
