...
import { Cpu, Database, Monitor, Server, Share2, Wifi } from "lucide-react";
...
interface TopoDevice {
  ...
  interfaces: {
    ...
    cableA: {
      ...
      label?: string | null;
      color?: string | null;
      interfaceB: { device: { id: string; name: string } };
    }[];
    cableB: {
      id: string;
      label?: string | null;
      color?: string | null;
      interfaceA: { device: { id: string; name: string } };
    }[];
  }[];
...
const TopologyView = ({ selectedCategory = null }: TopologyViewProps) => {
  ...
  const [posOverrides, setPosOverrides] =
    useState<Map<string, { x: number; y: number }>>(loadPositions);
      ...
      d.interfaces.forEach((iface) => {
        iface.cableA.forEach((cable) => {
          ...
          if (!edgeSet.has(key)) {
            ...
            edgeList.push({
              from: d.id,
              to: cable.interfaceB.device.id,
              label: cable.label,
              color: cable.color || "#94a3b8",
              type: "cable",
            });
          }
        });
  ...
  const selectedDevice = useMemo(
    () => devices.find((d) => d.id === selectedNode) || null,
  ...
  const handleSubnetMouseDown = useCallback(
    (e: React.MouseEvent, subnetId: string) => {
      e.stopPropagation();
      const pt = svgPoint(e.clientX, e.clientY);
      dragMovedRef.current = false;
  ...
  const centerCanvas = useCallback(() => {
    ...
    const newZoom = Math.min(
      1,
      Math.min((rect.width - 80) / contentW, (rect.height - 80) / contentH),
    );
    setPan({
    ...
    return (
      <div className="flex gap-0 h-full relative animate-fade-in">
        <div className="flex flex-col items-center justify-center p-16 px-8 text-center bg-(--surface) border border-border rounded-lg flex-1">
          ...
          <p>
            Add devices and connect them with cables to see your network
            topology here.
          </p>
        </div>
      </div>
    ...
    <div className="flex gap-0 h-full relative animate-fade-in">
        ...
        >
          <g transform={`translate(${pan.x}, ${pan.y}) scale(${zoom})`}>
            ...
            {devices.map((device) => {
              ...
              const subIdx = subId
                ? Array.from(subnetGroups.groups.keys()).indexOf(subId)
                : -1;
              const subColor =
                subIdx >= 0 ? subnetColors[subIdx % subnetColors.length] : null;
              ...
              return (
                <g
                  ...
                  style={{
                    cursor: dragging === device.id ? "grabbing" : "pointer",
                ...
                >
                  ...
                  <text
                    x={subColor && showSubnetClouds ? 24 : 12}
                    y={60}
                    fontSize={8}
                    fill="#94a3b8"
                  >
                    {device.services.length > 0
                      ? `${device.category} · ${device.services.length} svc`
                      : device.category}
                  </text>
                </g>
              );
      ...
      {selectedDevice && (
        <TopologyDetailPanel
          ...
          selectedSubnet={
            (deviceSubnetMap.get(selectedDevice.id)
              ? subnets.find(
                  (s) => s.id === deviceSubnetMap.get(selectedDevice.id),
                )
              : null) || null
          }
          peers={(() => {
