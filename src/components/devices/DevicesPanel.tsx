    ...
    return (
      <div className="flex-1 overflow-auto p-4 px-6">
        <div className="flex items-center justify-center h-[220px] text-muted-foreground text-[13px]">
          <Server size={16} className="mr-2 text-[#cbd5e1]" /> Loading
          devices...
        </div>
      </div>
    );
      ...
      <div className="flex-1 overflow-auto p-4 px-6">
        <div className="flex flex-col items-center justify-center py-16 text-center bg-card border border-border rounded-lg">
          ...
          <p className="text-[13px] text-muted-foreground mb-6 max-w-[360px]">
            Add your first device to start managing your infrastructure.
          </p>
    ...
    <>
      <div className="flex gap-8 px-6 py-4 bg-card border-b border-border">
        ...
        <div className="flex flex-col gap-1">
          ...
          <span className="text-lg font-bold text-(--blue)">
            {devices.filter((d) => d.category === "Networking").length}
          </span>
        </div>
        <div className="flex flex-col gap-1">
          <span className="text-[11px] uppercase font-semibold text-muted-foreground">
          ...
          <span className="text-lg font-bold text-(--blue)">
            {devices.filter((d) => ["VM", "LXC"].includes(d.category)).length}
          </span>
        </div>
        <div className="flex flex-col gap-1">
          <span className="text-[11px] uppercase font-semibold text-muted-foreground">
            ...
            <tbody>
              {filteredDevices.length === 0 ? (
                <tr>
                  <td
                    colSpan={8}
                    className="text-center h-[100px] text-muted-foreground"
                  >
                ...
                filteredDevices.map((device) => (
                  <tr
                    key={device.id}
                    data-highlight-id={device.id}
                    className={
                      highlightId === device.id ? "highlight-flash" : ""
                    }
                  >
                    <td className="text-center">
                      {(() => {
                        const cat = categories.find(
                          (c) => c.name === device.category,
                        );
                        return cat ? (
                          renderCategoryIcon(cat.icon, 14, cat.color)
                        ) : (
                          <Server size={14} className="text-(--text-slate)" />
                        );
                      })()}
                    </td>
                    ...
                    <td className="text-(--text-slate) font-mono text-xs">
                      {device.macAddress}
                    </td>
                    <td>
                      {(() => {
                    ...
                    <td className="text-(--text-slate) text-xs">
                      {device.platform || "—"}
                    </td>
