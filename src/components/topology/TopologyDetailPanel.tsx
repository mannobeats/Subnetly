    ...
    <div className="w-80 bg-(--surface) border border-border rounded-lg p-5 ml-4 overflow-y-auto max-h-full animate-fade-in">
      <div className="flex items-center gap-4 mb-5 pb-4 border-b border-border">
        <div
          ...
          style={{
            background: `${categoryColors[selectedDevice.category]}14`,
            color: categoryColors[selectedDevice.category],
          }}
        >
          {(() => {
            const I = categoryIcons[selectedDevice.category] || Monitor;
            return <I size={20} />;
          })()}
        </div>
        <div>
          <h3>{selectedDevice.name}</h3>
          <code className="text-[11px] text-(--text-slate)">
      ...
      <div className="flex flex-col gap-3 mb-5">
        ...
        <div className="flex items-center justify-between">
          ...
          <span
            className={`px-2 py-0.5 rounded text-[11px] font-semibold ${selectedDevice.status === "active" ? "bg-(--green-bg) text-(--green)" : "bg-(--orange-bg) text-(--orange)"}`}
          >
        ...
        {selectedDevice.deviceType && (
          <div className="flex items-center justify-between">
            ...
            <span className="text-xs">
              {selectedDevice.deviceType.manufacturer.name}{" "}
              {selectedDevice.deviceType.model}
            </span>
          </div>
        )}
        {selectedSubnet && (
          <div className="flex items-center justify-between">
            ...
            <span className="text-xs">
              {selectedSubnet.prefix}/{selectedSubnet.mask}
              {selectedSubnet.vlan ? ` (VLAN ${selectedSubnet.vlan.vid})` : ""}
            </span>
          </div>
        )}
      </div>
        ...
        <div className="mt-4 pt-4 border-t border-border">
          ...
          {selectedDevice.services.map((s) => (
            <div key={s.id} className="flex items-center justify-between py-1">
              ...
              <code className="text-[10px] bg-(--muted-bg) px-1.5 py-0.5 rounded">
                {s.protocol}:{s.ports}
              </code>
            </div>
          ))}
        ...
        <div className="mt-4 pt-4 border-t border-border">
          ...
          {peers.map((p) => (
            ...
            >
              ...
              <span>{p.name}</span>
              <code className="text-[9px] text-(--text-light) ml-auto">
                {p.ipAddress}
              </code>
            </div>
          ))}
