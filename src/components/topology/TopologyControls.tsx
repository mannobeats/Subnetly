...
export default function TopologyControls({
  ...
  return (
    <div className="absolute top-3 right-3 flex items-center gap-1 bg-(--surface) border border-border rounded-lg px-2 py-1 z-10 shadow-sm">
      ...
      <button
        className="flex items-center justify-center w-7 h-7 border-none bg-transparent rounded-md cursor-pointer text-(--text-slate) transition-all hover:bg-(--muted-bg-alt) hover:text-(--text)"
        onClick={onZoomIn}
        title="Zoom In"
      >
        <ZoomIn size={14} />
      ...
      <span className="text-[10px] text-(--text-light) font-medium min-w-8 text-center">
        {Math.round(zoom * 100)}%
      </span>
    </div>
  );
