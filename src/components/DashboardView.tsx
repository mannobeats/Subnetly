...
interface DashboardData {
  ...
  subnetStats: {
    ...
    usedIps: number;
    utilization: number;
    ranges: { role: string; startAddr: string; endAddr: string }[];
  }[];
  recentChanges: {
    id: string;
    objectType: string;
  ...
  wifiNetworks: {
    ...
    vlan: { vid: number; name: string } | null;
    subnet: { prefix: string; mask: number } | null;
  }[];
}
...
const DashboardView = ({ categories = [] }: DashboardViewProps) => {
  ...
  const statCards = [
    ...
    {
      label: "Subnets",
      value: data.counts.subnets,
      icon: Globe,
      color: "#10b981",
      sub: `${data.counts.ipAddresses} IPs tracked`,
    },
    {
      label: "VLANs",
      value: data.counts.vlans,
      icon: Network,
      color: "#7c3aed",
      sub: "Configured",
    },
    {
      label: "WiFi",
      value: data.counts.wifiNetworks || 0,
      icon: Wifi,
      color: "#06b6d4",
      sub: `${wifiEnabled} enabled`,
    },
    {
      label: "Services",
      ...
      icon: Box,
      color: "#f97316",
      sub: `${healthyCount} healthy${downCount > 0 ? ` · ${downCount} down` : ""}`,
    },
    {
      label: "Monitored",
      value: data.counts.monitored || 0,
      icon: Activity,
      color: "#22c55e",
        ...
        {statCards.map((s, i) => (
          ...
          >
            ...
            <div className="flex flex-col">
              ...
              <span className="text-[11px] text-(--text-light) mt-0.5">
                {s.sub}
              </span>
            </div>
          </div>
      ...
      <div className="bg-card border border-border rounded-lg p-5 mb-4">
        <div className="flex items-center justify-between mb-4">
          ...
          <span className="text-[11px] text-muted-foreground bg-(--muted-bg) px-2 py-0.5 rounded">
            {data.subnetStats.length} subnets
          </span>
        </div>
        <div className="grid grid-cols-2 gap-4 max-h-80 overflow-y-auto">
          {data.subnetStats.map((s) => (
            <div
              key={s.id}
              className="bg-(--surface-alt) border border-border rounded-md p-4"
            >
              <div className="flex items-center justify-between mb-2">
                <code className="text-[13px] font-semibold bg-(--muted-bg) px-2 py-0.5 rounded">
                  {s.prefix}
                </code>
                {s.vlan && (
              ...
              {s.ranges.length > 0 && (
                <div className="flex flex-wrap gap-1 mt-2">
                  {s.ranges.map((r, i) => (
                    <span
                      key={i}
                      className={`px-2 py-0.5 rounded text-[11px] font-semibold ${r.role === "dhcp" ? "bg-(--orange-bg) text-(--orange)" : r.role === "reserved" ? "bg-(--purple-bg) text-(--purple)" : "bg-(--blue-bg) text-(--blue)"}`}
                    >
                      {r.role}: {r.startAddr.split(".").pop()}-
                      {r.endAddr.split(".").pop()}
                    </span>
                  ))}
                </div>
                ...
                <div key={cat} className="flex items-center justify-between">
                  <div className="flex items-center gap-2 text-xs font-medium min-w-[100px]">
                    <div
                      ...
                      style={{ background: categoryColorMap[cat] || "#64748b" }}
                    />
                    <span>{cat}</span>
                  </div>
                  <div className="flex items-center gap-3 flex-1 ml-4">
                    <div className="flex-1 h-1.5 bg-(--muted-bg) rounded-full overflow-hidden">
                      <div
                        className="h-full rounded-full transition-all duration-600"
                        style={{
                          width: `${(count / data.counts.devices) * 100}%`,
                          background: categoryColorMap[cat] || "#64748b",
                        }}
                      />
                    </div>
                    <span className="text-xs font-semibold min-w-5 text-right">
                      {count}
                    </span>
                  </div>
                </div>
            ...
            {data.recentChanges.map((c) => {
              ...
              return (
                <div key={c.id} className="flex items-center gap-3">
                  <div
                    className={`w-7 h-7 rounded-md flex items-center justify-center shrink-0 ${isCreate ? "bg-(--green-bg) text-(--green)" : "bg-(--blue-bg) text-(--blue)"}`}
                  >
                    {isCreate ? (
      ...
      <div className="grid grid-cols-2 gap-4 mb-4">
        ...
        <div className="bg-card border border-border rounded-lg p-5">
          <div className="flex items-center justify-between mb-4">
            ...
            <span className="text-[11px] text-muted-foreground bg-(--muted-bg) px-2 py-0.5 rounded">
              {(data.wifiNetworks || []).length} networks
            </span>
          </div>
            ...
            {(data.wifiNetworks || []).length === 0 ? (
              ...
              <div className="flex flex-col gap-2">
                {(data.wifiNetworks || []).map((w) => {
                  const secColor = w.security.startsWith("wpa3")
                    ? "#10b981"
                    : w.security.startsWith("wpa2")
                      ? "#3366ff"
                      : w.security === "open"
                        ? "#ef4444"
                        : "#64748b";
                  return (
                    <div
                      key={w.id}
                      className="flex items-center gap-3 px-3 py-2 rounded-lg bg-(--surface-alt) border border-border"
                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="text-[10px] text-muted-foreground flex gap-1.5 items-center">
                          ...
                          <span>·</span>
                          <span>
                            {w.band === "both" ? "2.4+5 GHz" : w.band}
                          </span>
                          {w.vlan && (
                            <>
                              <span>·</span>
                              <span>VLAN {w.vlan.vid}</span>
                            </>
                          )}
                        </div>
                      </div>
                      <span
                        className={`px-2 py-0.5 rounded text-[9px] font-semibold ${w.enabled ? "bg-(--green-bg) text-(--green)" : "bg-(--orange-bg) text-(--orange)"}`}
                      >
                        {w.enabled ? "on" : "off"}
                      </span>
                      {w.guestNetwork && (
                        <span className="px-1 py-px rounded text-[9px] bg-(--orange-bg) text-(--orange)">
                          Guest
                        </span>
                      )}
                    </div>
        ...
        <div className="bg-card border border-border rounded-lg p-5">
          ...
          <div className="grid grid-cols-[repeat(auto-fill,minmax(200px,1fr))] gap-3 max-h-70 overflow-y-auto">
            {data.services.map((s) => {
              const dotColor =
                healthDotColors[s.healthStatus] || healthDotColors.unknown;
              return (
                <div
                  key={s.id}
                  className="bg-(--surface-alt) border border-border rounded-md p-3 relative"
                >
                  ...
                  <div className="flex items-center gap-1.5 flex-wrap text-[11px] mt-1">
                    <code className="bg-(--muted-bg) px-1.5 py-px rounded text-[10px]">
                    ...
                    {s.isDocker && (
                      <span className="text-[9px] bg-[#dbeafe] text-[#1e40af] px-1 py-px rounded">
                        Docker
                      </span>
                    )}
