...
import type {
  ...
  IPRange,
  IPRangeScheme,
  SubnetTemplate as PersistedSubnetTemplate,
} from "@/types";
...
function getSubnetBlock(
  prefix: string,
  mask: number,
): { base: string; startOctet: number; blockSize: number } {
...
function getSuggestedRangeStartOctet(
  subnet: SubnetWithRelations,
  devices: Device[],
): string {
...
const IPPlannerView = ({
  ...
  const [vlans, setVlans] = useState<
    { id: string; vid: number; name: string }[]
  >([]);
  const [devices, setDevices] = useState<Device[]>([]);
  const [customSubnetTemplates, setCustomSubnetTemplates] = useState<
    PersistedSubnetTemplate[]
  >([]);
  const [rangeSchemes, setRangeSchemes] = useState<IPRangeScheme[]>([]);
  const [subnetRoleCategories, setSubnetRoleCategories] = useState<
    Array<{ id: string; name: string; slug: string; color?: string }>
  >([]);
  const [ipRangeRoleCategories, setIpRangeRoleCategories] = useState<
    Array<{ id: string; name: string; slug: string; color?: string }>
  >([]);
  ...
  const [dynamicRangeStartEnabled, setDynamicRangeStartEnabled] =
    useState(true);
  ...
  const fetchData = useCallback(() => {
    Promise.all([
      ...
      fetch("/api/categories?type=subnet_role").then((r) =>
        r.ok ? r.json() : [],
      ),
      fetch("/api/categories?type=ip_range_role").then((r) =>
        r.ok ? r.json() : [],
      ),
    ])
      .then(
        ([
          ...
          setCustomSubnetTemplates(
            Array.isArray(subnetTemplateData) ? subnetTemplateData : [],
          );
          setRangeSchemes(
            Array.isArray(rangeSchemeData) ? rangeSchemeData : [],
          );
          setSubnetRoleCategories(
            Array.isArray(subnetRoleData) ? subnetRoleData : [],
          );
          setIpRangeRoleCategories(
            Array.isArray(ipRangeRoleData) ? ipRangeRoleData : [],
          );
          setSelectedSubnet((prev) => {
            if (
              prev &&
              subnetData.some((s: SubnetWithRelations) => s.id === prev)
            )
              return prev;
    ...
    const loadSettings = () => {
      try {
        const saved =
          ...
          localStorage.getItem(LEGACY_APP_SETTINGS_KEY);
        if (!saved) return;
  ...
  const subnet = useMemo(
    () => subnets.find((s) => s.id === selectedSubnet),
  ...
  const rangeRoleSuggestions = useMemo(() => {
    const roles = new Set<string>(
      ipRangeRoleCategories.map((role) => role.name),
    );
    subnets.forEach((s) => s.ipRanges.forEach((r) => roles.add(r.role)));
    rangeSchemes.forEach((scheme) =>
      scheme.entries.forEach((entry) => roles.add(entry.role)),
    );
  ...
  const subnetRoleSuggestions = useMemo(() => {
    const roles = new Set<string>(
      subnetRoleCategories.map((role) => role.name),
    );
    subnets.forEach((s) => {
      if (s.role) roles.add(s.role);
    });
    subnetTemplateOptions.forEach((t) => {
      if (t.role) roles.add(t.role);
    });
    return Array.from(roles);
  }, [subnets, subnetTemplateOptions, subnetRoleCategories]);

  const persistCustomRoleCategory = useCallback(
    async (name: string, type: "subnet_role" | "ip_range_role") => {
  ...
  const normalizeRangeRoleKey = useCallback(
    (value: string) =>
      value
        .trim()
        .toLowerCase()
  ...
  const getRangeRoleVisual = useCallback(
    (role: string) => {
      const normalized = normalizeRangeRoleKey(role);
      const category = ipRangeRoleCategories.find(
        (item) =>
  ...
  const subnetMaps = useMemo(() => {
    if (!subnet)
      return {
        ipMap: new Map<number, IPAddress>(),
        rangeMap: new Map<number, IPRange>(),
      };
  ...
  const cellData = useMemo(() => {
    ...
    const cells = [];
    const gatewayOctet = subnet.gateway
      ? parseInt(subnet.gateway.split(".").pop() || "1")
      : startOctet + 1;
  ...
  const filteredCells = useMemo(() => {
    ...
    return cellData.map((c) => ({
      ...c,
      highlighted:
        c.ip?.dnsName?.toLowerCase().includes(searchTerm.toLowerCase()) ||
      ...
      for (let j = i + 1; j < subnets.length; j++) {
        ...
        const intB =
          ((partsB[0] << 24) |
            ...
            partsB[3]) >>>
          0;
        const sizeA = 2 ** (32 - sa.mask);
        const sizeB = 2 ** (32 - sb.mask);
        ...
        if (intA <= endB && intB <= endA) {
          overlaps.push({
            a: `${sa.prefix}/${sa.mask}`,
            b: `${sb.prefix}/${sb.mask}`,
            aDesc: sa.description || "Unnamed",
            bDesc: sb.description || "Unnamed",
          });
        }
      }
    }
  ...
  const handleSaveSubnet = async (e: React.FormEvent) => {
    ...
    if (isCustomSubnetRole && normalizedRole) {
      const persisted = await persistCustomRoleCategory(
        normalizedRole,
        "subnet_role",
      );
    ...
    const url = editingSubnetId
      ? `/api/subnets/${editingSubnetId}`
      : "/api/subnets";
    const payload = {
    ...
    if (res.ok) {
      ...
      if (!editingSubnetId && subnetTemplatesEnabled && saveAsTemplate) {
        const tRes = await fetch("/api/subnet-templates", {
          ...
          body: JSON.stringify({
            name:
              templateName.trim() || `${subnetForm.prefix}/${subnetForm.mask}`,
            prefix: subnetForm.prefix,
            mask: parseInt(subnetForm.mask, 10),
            gateway: subnetForm.gateway || null,
        ...
        if (!tRes.ok) {
          alert(
            "Subnet created, but saving template failed (possibly duplicate template name).",
          );
        }
      }
      ...
      alert(
        editingSubnetId ? "Failed to update subnet" : "Failed to create subnet",
      );
    }
  };
  ...
  const openEditSubnet = () => {
    ...
    setIsCustomSubnetRole(
      !!subnet.role && !subnetRoleSuggestions.includes(subnet.role),
    );
  ...
  const applySubnetTemplate = (templateId: string) => {
    ...
    setSubnetForm((prev) => ({
      ...
      prefix: template.prefix,
      mask: template.mask,
      gateway: smartGatewayEnabled
        ? suggestGatewayFromPrefix(
            template.prefix,
            parseInt(template.mask, 10),
          ) || template.gateway
        : template.gateway,
      role: template.role,
      description: template.description,
  ...
  const updateSubnetFormWithSmartGateway = (
    updates: Partial<typeof emptySubnetForm>,
  ) => {
    setSubnetForm((prev) => {
      ...
      if (
        !editingSubnetId &&
        smartGatewayEnabled &&
        updates.gateway === undefined &&
        !prev.gateway.trim()
      ) {
        const suggested = suggestGatewayFromPrefix(
          next.prefix,
          parseInt(next.mask, 10),
        );
  ...
  const handleDeleteSubnet = async () => {
    ...
    const res = await fetch(`/api/subnets/${selectedSubnet}`, {
      method: "DELETE",
    });
    if (res.ok) {
      setDeleteSubnetModal(false);
  ...
  const handleSaveRange = async (e: React.FormEvent) => {
    ...
    const url = editingRangeId
      ? `/api/ranges/${editingRangeId}`
      : "/api/ranges";
    const res = await fetch(url, {
      method,
      headers: { "Content-Type": "application/json" },
    ...
    if (res.ok) {
      ...
      alert(
        editingRangeId ? "Failed to update range" : "Failed to create range",
      );
    }
  };
  ...
  const openEditRange = (range: IPRange) => {
    ...
    setIsCustomRangeRole(
      !!range.role && !rangeRoleSuggestions.includes(range.role),
    );
  ...
  const openCreateRange = () => {
    ...
    if (dynamicRangeStartEnabled && subnet) {
      setRangeForm({
        ...emptyRangeForm,
        startOctet: getSuggestedRangeStartOctet(subnet, devices),
      });
    } else {
      setRangeForm(emptyRangeForm);
    }
  ...
  const handleAssignIp = async (e: React.FormEvent) => {
    ...
    const newDevice = ipForm.deviceId
      ? devices.find((d) => d.id === ipForm.deviceId)
      : null;

    // Find the device currently linked to THIS IP address (the one we're editing)
    const deviceOnThisIp =
      devices.find((d) => d.ipAddress === ipForm.address) || null;

    // If the new device already has a DIFFERENT IP, we need to clean up that old assignment
    const newDeviceOldIp =
      newDevice?.ipAddress && newDevice.ipAddress !== ipForm.address
        ? newDevice.ipAddress
        : null;
    ...
    if (res.ok) {
      ...
      if (newDeviceOldIp) {
        ...
        for (const s of subnets) {
          const found = s.ipAddresses.find(
            (ip) => ip.address === newDeviceOldIp,
          );
          if (found) {
            oldIpamId = found.id;
            break;
      ...
      alert(editingIpId ? "Failed to update IP" : "Failed to assign IP");
    }
  };
  ...
  const getCellColor = (status: string) => {
    ...
    switch (status) {
      case "network":
        return { bg: isDark ? "#000000" : "#1e293b", color: "#fff" };
      case "broadcast":
        return { bg: isDark ? "#000000" : "#1e293b", color: "#fff" };
      case "gateway":
        return {
          bg: root?.getPropertyValue("--green").trim() || "#10b981",
          color: "#fff",
        };
      case "assigned":
        return {
          bg: root?.getPropertyValue("--blue").trim() || "#3366ff",
          color: "#fff",
        };
      case "dhcp":
        return {
      ...
      case "infrastructure":
        return {
          bg: isDark ? "rgba(34, 211, 238, 0.15)" : "#cffafe",
          color: isDark ? "#22d3ee" : "#155e75",
        };
      default: {
        const custom = ipRangeRoleCategories.find(
          (item) =>
            item.slug === normalizeRangeRoleKey(status) ||
            item.name.toLowerCase() === status.toLowerCase(),
        );
        if (custom?.color) {
          return {
            bg: isDark ? `${custom.color}33` : `${custom.color}22`,
            color: custom.color,
          };
        }
        return {
          bg: root?.getPropertyValue("--muted-bg").trim() || "#f1f3f5",
          color: root?.getPropertyValue("--text-faint").trim() || "#adb5bd",
        };
      }
    }
  };
      ...
      <div className="p-0 animate-fade-in">
        <div className="flex flex-col items-center justify-center py-16 px-8 text-center bg-(--surface) border border-border rounded-lg">
          ...
          <p className="text-[13px] text-muted-foreground mb-6 max-w-[360px]">
            Create your first subnet to start planning IP addresses.
          </p>
          <Button onClick={openCreateSubnet}>
            <Plus size={14} /> Create Subnet
          </Button>
        </div>
    ...
    return (
      ...
      >
        <DialogContent className="max-w-[720px]">
          <DialogHeader>
            <DialogTitle>
              {editingSubnetId ? "Edit Subnet" : "Create Subnet"}
            </DialogTitle>
            {!editingSubnetId && subnetTemplatesEnabled && (
              <DialogDescription>
                Start from a template to speed up subnet planning. You can
                adjust any field before saving.
              </DialogDescription>
            )}
          </DialogHeader>
          <form onSubmit={handleSaveSubnet} className="space-y-4 mt-2">
                ...
                >
                  ...
                  {subnetTemplateOptions.map((template) => (
                    <option key={template.id} value={template.id}>
                      {template.name} — {template.prefix}/{template.mask}
            ...
            <div className="grid grid-cols-[2fr_1fr] gap-5">
              <div className="space-y-1.5">
                ...
                <Input
                  required
                  value={subnetForm.prefix}
                  onChange={(e) =>
                    updateSubnetFormWithSmartGateway({ prefix: e.target.value })
                  }
                  placeholder="e.g. 10.0.10.0"
              ...
              <div className="space-y-1.5">
                ...
                <select
                  ...
                  onChange={(e) =>
                    updateSubnetFormWithSmartGateway({ mask: e.target.value })
                  }
                >
            ...
            <div className="grid grid-cols-2 gap-5">
              <div className="space-y-1.5">
                ...
                {!editingSubnetId &&
                  smartGatewayEnabled &&
                  !subnetForm.gateway.trim() && (
                    <p className="text-[11px] text-(--text-muted)">
                      Gateway will auto-suggest as first usable host when
                      possible.
                    </p>
                  )}
              </div>
              <div className="space-y-1.5">
                ...
                <select
                  ...
                  onChange={(e) =>
                    setSubnetForm({ ...subnetForm, vlanId: e.target.value })
                  }
                >
            ...
            <div className="grid grid-cols-2 gap-5">
              <div className="space-y-1.5">
                ...
                {isCustomSubnetRole && (
                  <Input
                    value={subnetForm.role}
                    onChange={(e) =>
                      setSubnetForm({ ...subnetForm, role: e.target.value })
                    }
                    placeholder="Custom subnet role"
                    className="h-9 text-[13px]"
                  />
                )}
              </div>
                ...
                {saveAsTemplate && (
                  <div className="mt-2">
                    ...
                    <Input
                      ...
                      placeholder="e.g. Core Infrastructure /24"
                      className="h-9 text-[13px]"
                    />
                  </div>
                )}
            ...
            <DialogFooter className="mt-6">
              ...
              <Button type="submit">
                {editingSubnetId ? "Save Changes" : "Create Subnet"}
              </Button>
            </DialogFooter>
          </form>
        </DialogContent>
    ...
    <div className="p-0 animate-fade-in">
      ...
      <div className="flex items-center gap-6 mb-4 bg-(--surface) border border-border rounded-lg py-3.5 px-5">
        <div className="flex items-center gap-6 flex-1 min-w-0">
          ...
          <div className="flex-1 max-w-[300px]">
            ...
            <span className="text-[11px] text-(--text-muted)">
              {utilization.used} / {utilization.total} addresses used
            </span>
          </div>
        </div>
        <div className="flex gap-2 items-center shrink-0">
          <div className="ipam-view-toggle">
            <button
              className={`ipam-toggle-btn ${viewMode === "grid" ? "active" : ""}`}
            ...
            <button
              className={`ipam-toggle-btn ${viewMode === "list" ? "active" : ""}`}
            ...
            <button
              className={`ipam-toggle-btn ${viewMode === "summary" ? "active" : ""}`}
          ...
          {subnet && (
            <Button variant="outline" onClick={openCreateRange}>
              <Plus size={14} /> Range
            </Button>
          )}
          {subnet && rangeSchemesEnabled && (
            <Button
              variant="outline"
              onClick={() => {
                if (!selectedSchemeId && rangeSchemes.length > 0)
                  setSelectedSchemeId(rangeSchemes[0].id);
          ...
          {subnet && (
            <Button
              ...
              onClick={() => setDeleteSubnetModal(true)}
            >
              <Trash2 size={14} />
            </Button>
          )}
        ...
        <div className="mb-3 px-4 py-3 bg-[#fef2f2] border border-[#fecaca] rounded-lg">
          <div className="flex items-center gap-2 mb-2">
            ...
            <span className="font-semibold text-[13px] text-[#dc2626]">
              Subnet Overlap Detected ({subnetOverlaps.length})
            </span>
          </div>
          <div className="flex flex-col gap-1">
            {subnetOverlaps.map((o, i) => (
              <div
                key={i}
                className="text-xs text-[#991b1b] flex gap-1.5 items-center"
              >
                <code className="bg-[#fee2e2] px-1.5 py-px rounded text-[11px]">
                  {o.a}
                </code>
                <span>({o.aDesc})</span>
        ...
        <div className="flex flex-wrap gap-2 mb-4">
          {subnet.ipRanges.map((r, i) => {
            ...
            return (
              <div
                key={i}
                className="flex items-center gap-2 py-1.5 px-3 rounded-md border text-[11px] font-medium"
              ...
              >
                ...
                />
                <span>
                  {rc.label}: .{r.startAddr.split(".").pop()} — .
                  {r.endAddr.split(".").pop()}
                </span>
                {r.description && (
        ...
        <>
          ...
          {totalPages > 1 && (
            <div className="flex items-center justify-between mb-3 px-3 py-2 bg-card border border-border rounded-lg">
              <span className="text-xs text-muted-foreground">
                Showing addresses{" "}
                <strong>
                  {subnetBlock.startOctet + gridPage * GRID_PAGE_SIZE}
                </strong>{" "}
                —{" "}
                <strong>
                  {Math.min(
                    subnetBlock.startOctet +
                      (gridPage + 1) * GRID_PAGE_SIZE -
                      1,
                    subnetBlock.startOctet + subnetBlock.blockSize - 1,
                  )}
                </strong>{" "}
                of <strong>{subnetBlock.blockSize}</strong>
              </span>
              <div className="flex items-center gap-2">
                ...
                <div className="flex gap-0.5">
                  {Array.from({ length: Math.min(totalPages, 10) }, (_, i) => {
                    const pageIdx =
                      totalPages <= 10
                        ...
                        : gridPage < 5
                          ? i
                          : gridPage > totalPages - 6
                            ? totalPages - 10 + i
                            : gridPage - 4 + i;
                    return (
                      <button
                        key={pageIdx}
                        className={`px-2 py-1 text-[11px] rounded min-w-7 ${pageIdx === gridPage ? "bg-[#3366ff] text-white font-bold" : "bg-transparent hover:bg-accent"}`}
                        onClick={() => setGridPage(pageIdx)}
                      >
                        {pageIdx + 1}
                      </button>
                    );
                  ...
                  {totalPages > 10 && (
                    <span className="text-[11px] text-(--text-light) p-1">
                      of {totalPages}
                    </span>
                  )}
                </div>
          ...
          <div className="bg-(--surface) border border-border rounded-lg p-5 mb-6">
            <div
              className="ipam-grid"
              style={{
                gridTemplateColumns: `repeat(${gridColumns(subnet.mask)}, 1fr)`,
              }}
            >
              {filteredCells.map((cell) => {
                const colors = getCellColor(cell.status);
                const isHighlighted = "highlighted" in cell && cell.highlighted;
                const ipFilterDimmed = selectedIpFilter
                  ? cell.status !== selectedIpFilter
                  : false;
                const dimmed =
                  (searchTerm && !isHighlighted && cell.status !== "gateway") ||
                  ipFilterDimmed;
                const label =
                  cell.device?.name || cell.ip?.dnsName || cell.ip?.assignedTo;
                return (
                  <div
                    key={cell.octet}
                    ...
                    style={{
                      ...
                      cursor:
                        cell.status !== "network" && cell.status !== "broadcast"
                          ? "pointer"
                          : "default",
                    }}
                    ...
                    onClick={() => {
                      if (
                        ...
                        (!!cell.range && !cell.ip && !cell.device)
                      ) {
                        openAssignFromGrid(cell.fullIp);
                      } else if (cell.ip) {
                        const linkedDev =
                          cell.device ||
                          devices.find((d) => d.ipAddress === cell.ip!.address);
                        setEditingIpId(cell.ip.id);
                        setIpForm({
                          address: cell.ip.address,
                          dnsName: cell.ip.dnsName || "",
                          description: cell.ip.description || "",
                          status: cell.ip.status,
                          deviceId: linkedDev?.id || "",
                        });
                        setIpModalOpen(true);
                        ...
                        setIpForm({
                          address: cell.fullIp,
                          dnsName: cell.device.name,
                          description: `Assigned to ${cell.device.name}`,
                          status: "active",
                  ...
                  >
                    ...
                    {label && (
                      <span className="ipam-cell-label">
              ...
              return (
                <div className="ipam-tooltip">
                  ...
                  <div className="ipam-tooltip-content">
                    ...
                    {cell.isGateway && (
                      <span className="px-2 py-0.5 rounded text-[11px] font-semibold bg-(--green-bg) text-(--green)">
                        Gateway
                      </span>
                    )}
                    {cell.device && (
                    ...
                    {cell.ip && !cell.device && (
                      <span>
                        {cell.ip.dnsName || cell.ip.assignedTo || "Unnamed"}
                      </span>
                    )}
                    {cell.ip?.description && (
                      <span className="ipam-tooltip-desc">
                        {cell.ip.description}
                      </span>
                    )}
                    {!cell.ip &&
                    ...
                    {(cell.status === "available" ||
                      (!!cell.range && !cell.ip && !cell.device)) && (
                      <span className="ipam-tooltip-action">
                        Click to assign
                      </span>
                    )}
                    {(cell.ip || cell.device) && cell.status === "assigned" && (
                      <span className="ipam-tooltip-action">
                        Click to edit / unassign
                      </span>
                    )}
                  </div>
                </div>
      ...
      {viewMode === "list" && subnet && (
        <div className="bg-card border border-border rounded-lg overflow-hidden">
          <table className="w-full border-collapse table-fixed">
            ...
            <tbody>
              {filteredCells
                .filter((c) => c.status !== "available" || searchTerm)
                .slice(0, searchTerm ? undefined : 256)
                .map((cell) => {
                  const isHighlighted =
                    "highlighted" in cell && cell.highlighted;
                  ...
                  return (
                    <tr key={cell.octet} style={{ opacity: dimmed ? 0.3 : 1 }}>
                      <td>
                        <span className="font-semibold text-[11px] text-(--text-light)">
                          .{cell.octet}
                        </span>
                      </td>
                      <td>
                        <code className="text-[11px] bg-(--muted-bg) px-1.5 py-px rounded">
                          {cell.fullIp}
                        </code>
                      </td>
                      <td>
                        <span
                          className="ipam-list-status"
                        ...
                        >
                          {cell.status === "gateway"
                            ...
                            : cell.status === "assigned"
                              ...
                              : cell.status === "network"
                                ? "Network"
                                : cell.status === "broadcast"
                                  ? "Broadcast"
                                  : cell.range
                                    ? getRangeRoleVisual(cell.range.role).label
                                    : "Available"}
                        </span>
                      </td>
                      ...
                      <td className="text-[11px] text-(--text-light)">
                        {cell.range
                          ? `${getRangeRoleVisual(cell.range.role).label}: .${cell.range.startAddr.split(".").pop()}–.${cell.range.endAddr.split(".").pop()}`
                          : "—"}
                      </td>
                        ...
                        {cell.ip && (
                          <div className="flex gap-0.5 justify-end">
                            <Button
                              variant="ghost"
                              size="icon"
                              className="h-6 w-6 text-[#3366ff]"
                              onClick={() => {
                                setEditingIpId(cell.ip!.id);
                                const dev =
                                  cell.device ||
                                  devices.find(
                                    (d) => d.ipAddress === cell.ip!.address,
                                  );
                                setIpForm({
                                  address: cell.ip!.address,
                                  dnsName: cell.ip!.dnsName || "",
                            ...
                            <Button
                              ...
                              onClick={() => handleDeleteIp(cell.ip!.id)}
                              title="Delete"
                            >
                              <Trash2 size={12} />
                            </Button>
                        ...
                        {!cell.ip && cell.device && (
                          <div className="flex gap-0.5 justify-end">
                            <Button
                              ...
                              className="h-6 w-6 text-[#3366ff]"
                              onClick={() =>
                                handlePromoteDeviceIp(cell.device!)
                              }
                              title="Manage in IPAM"
                            >
                              <Edit2 size={12} />
                            </Button>
                            <Button
                              ...
                              className="h-6 w-6 text-(--red)"
                              onClick={() =>
                                handleUnlinkDevice(cell.device!.id)
                              }
                              title="Unlink IP"
                            >
                              <Trash2 size={12} />
                        ...
                        {!cell.ip &&
                          ...
                          cell.status !== "gateway" && (
                            <Button
                              ...
                              size="icon"
                              className="h-6 w-6 text-[#3366ff]"
                              onClick={() => openAssignFromGrid(cell.fullIp)}
                            >
                              <Plus size={12} />
                            </Button>
                          )}
      ...
      {viewMode === "summary" &&
        ...
        (() => {
          ...
          const rangeSummary = subnet.ipRanges.map((r) => {
            ...
            const used = cellData.filter(
              (c) => c.octet >= start && c.octet <= end && (c.ip || c.device),
            ).length;
            ...
            <div className="flex flex-col gap-5">
              ...
              <div className="grid grid-cols-4 gap-4">
                <div className="bg-card border border-border rounded-lg p-5">
                  ...
                  <div className="text-2xl font-bold leading-none text-[#3366ff]">
                    {utilization.used}
                  </div>
                  <div className="text-[10px] text-(--text-light)">
                    {utilization.pct}% of {utilization.total}
                  </div>
                </div>
                <div className="bg-card border border-border rounded-lg p-5">
                  <div className="text-xs text-(--text-muted) font-medium">
                  ...
                  <div className="text-2xl font-bold leading-none text-[#10b981]">
                ...
                <div className="bg-card border border-border rounded-lg p-5">
                  ...
                  <div className="text-2xl font-bold leading-none text-[#7c3aed]">
                    {deviceCount}
                  </div>
                  <div className="text-[10px] text-(--text-light)">
                    Linked to IPs
                ...
                <div className="bg-card border border-border rounded-lg p-5">
                  ...
                  <div className="text-2xl font-bold leading-none text-[#f59e0b]">
                    {ipamCount}
                  </div>
                  <div className="text-[10px] text-(--text-light)">
                    No device linked
              ...
              <div className="bg-(--surface) border border-border rounded-[10px] p-5">
                ...
                <div className="flex h-5 rounded-md overflow-hidden gap-px mb-3">
                  {statusCounts["gateway"] && (
                    <div
                      className="min-w-0.5 transition-all duration-300"
                      style={{
                        flex: statusCounts["gateway"],
                        background: "#10b981",
                      }}
                      title={`Gateway: ${statusCounts["gateway"]}`}
                    />
                  )}
                  ...
                  {statusCounts["dhcp"] && (
                    <div
                      className="min-w-0.5 transition-all duration-300"
                      style={{
                        flex: statusCounts["dhcp"],
                        background: "#f59e0b",
                      }}
                      title={`DHCP: ${statusCounts["dhcp"]}`}
                    />
                  )}
                  {statusCounts["reserved"] && (
                    <div
                      className="min-w-0.5 transition-all duration-300"
                      style={{
                        flex: statusCounts["reserved"],
                        background: "#8b5cf6",
                      }}
                      title={`Reserved: ${statusCounts["reserved"]}`}
                    />
                  )}
                  {statusCounts["infrastructure"] && (
                    <div
                      className="min-w-0.5 transition-all duration-300"
                      style={{
                        flex: statusCounts["infrastructure"],
                        background: "#06b6d4",
                      }}
                      title={`Infrastructure: ${statusCounts["infrastructure"]}`}
                    />
                  )}
                ...
                <div className="flex flex-wrap gap-3 text-[11px] text-(--text-slate)">
                  ...
                  {statusCounts["infrastructure"] && (
                    <span>
                      ...
                      />{" "}
                      Infrastructure ({statusCounts["infrastructure"]})
                    </span>
                  )}
                  <span>
                    <span
                  ...
                  <div className="flex flex-col gap-2">
                    {cellData
                      ...
                      .map((c) => (
                        <div
                          key={c.octet}
                          className="flex items-center gap-2 py-2 px-3 bg-(--surface-alt) border border-(--muted-bg-alt) rounded-lg text-xs"
                        >
                          <Server size={13} color="#3366ff" />
                          <span className="font-medium">{c.device!.name}</span>
                          <code className="text-[10px] text-(--text-light) ml-auto">
                            {c.fullIp}
                          </code>
                          <span
                            className={`px-2 py-0.5 rounded text-[9px] font-semibold ${c.device!.status === "active" ? "bg-(--green-bg) text-(--green)" : "bg-(--orange-bg) text-(--orange)"}`}
                          >
                            {c.device!.status}
                          </span>
                        </div>
      ...
      {viewMode === "grid" &&
        ...
        (() => {
          ...
          const ipamAddresses = new Set(
            subnet.ipAddresses.map((ip) => ip.address),
          );
          const { base, startOctet, blockSize } = subnetBlock;
          const deviceOnlyEntries = devices.filter((d) => {
            ...
            return (
              lastOctet >= startOctet && lastOctet < startOctet + blockSize
            );
          });
          const totalAssigned =
            subnet.ipAddresses.length + deviceOnlyEntries.length;
          return (
            <div className="bg-card border border-border rounded-lg overflow-hidden">
              <div className="flex items-center justify-between px-5 py-4">
                <h2 className="text-sm font-semibold">Assigned Addresses</h2>
                <div className="flex gap-2 items-center">
                  <span className="text-[11px] text-(--text-muted) bg-(--muted-bg) px-2 py-0.5 rounded">
                    {totalAssigned} addresses
                  </span>
                  <Button
                    size="sm"
                  ...
                  >
                    <Plus size={12} /> Assign IP
                  </Button>
                </div>
              </div>
              {totalAssigned === 0 ? (
                <div className="p-8 text-center text-muted-foreground text-[13px]">
                  No IP addresses assigned yet. Click a cell in the grid or use
                  the button above.
                </div>
              ) : (
                <table className="w-full border-collapse table-fixed">
                  ...
                  <tbody>
                    {subnet.ipAddresses
                      .sort(
                        (a, b) =>
                          parseInt(a.address.split(".").pop() || "0") -
                          parseInt(b.address.split(".").pop() || "0"),
                      )
                      .map((ip) => {
                    ...
                    {deviceOnlyEntries.map((d) => (
                      <tr key={`dev-${d.id}`}>
                        ...
                        <td>
                          <span
                            className={`px-2 py-0.5 rounded text-[11px] font-semibold ${d.status === "active" ? "bg-(--green-bg) text-(--green)" : "bg-(--orange-bg) text-(--orange)"}`}
                          >
                            {d.status}
                          </span>
                        </td>
                        <td className="text-muted-foreground">
                          {d.category} — {d.platform || "No platform"}
                        </td>
                        <td className="text-right pr-2">
                          <div className="flex gap-0.5 justify-end">
                            <Button
                              ...
                              size="icon"
                              className="h-6 w-6 text-[#3366ff]"
                              onClick={() => handlePromoteDeviceIp(d)}
                              title="Manage in IPAM"
                            >
                              <Edit2 size={12} />
                            </Button>
                            <Button
                              ...
                              size="icon"
                              className="h-6 w-6 text-(--red)"
                              onClick={() => handleUnlinkDevice(d.id)}
                              title="Unlink IP from device"
                            >
                              <Trash2 size={12} />
                            </Button>
                          </div>
      ...
      >
        <DialogContent>
          <DialogHeader>
            <DialogTitle>
              {editingRangeId ? "Edit" : "Add"} IP Range{" "}
              {editingRangeId ? "" : `to ${subnet?.prefix}/${subnet?.mask}`}
            </DialogTitle>
          </DialogHeader>
          <form onSubmit={handleSaveRange} className="space-y-4 mt-2">
            <div className="grid grid-cols-2 gap-5">
              <div className="space-y-1.5">
                ...
                <Input
                  ...
                  min={1}
                  max={254}
                  value={rangeForm.startOctet}
                  onChange={(e) =>
                    setRangeForm({ ...rangeForm, startOctet: e.target.value })
                  }
                  placeholder="e.g. 150"
                  className="h-9 text-[13px]"
                />
              ...
              <div className="space-y-1.5">
                ...
                <Input
                  ...
                  min={1}
                  max={254}
                  value={rangeForm.endOctet}
                  onChange={(e) =>
                    setRangeForm({ ...rangeForm, endOctet: e.target.value })
                  }
                  placeholder="e.g. 199"
                  className="h-9 text-[13px]"
                />
            ...
            <div className="grid grid-cols-2 gap-5">
              <div className="space-y-1.5">
                ...
                {isCustomRangeRole && (
                  <Input
                    value={rangeForm.role}
                    onChange={(e) =>
                      setRangeForm({ ...rangeForm, role: e.target.value })
                    }
                    placeholder="Custom range role"
                    className="h-9 text-[13px]"
                  />
                )}
              </div>
              <div className="space-y-1.5">
                ...
                <Input
                  ...
                  onChange={(e) =>
                    setRangeForm({ ...rangeForm, description: e.target.value })
                  }
                  placeholder="Optional"
                  className="h-9 text-[13px]"
                />
            ...
            <DialogFooter className="mt-6">
              ...
              <Button type="submit">
                {editingRangeId ? "Save Changes" : "Add Range"}
              </Button>
            </DialogFooter>
          </form>
        </DialogContent>
      ...
      <Dialog
        open={saveSchemeModalOpen && !!subnet}
        onOpenChange={setSaveSchemeModalOpen}
      >
        <DialogContent>
          <DialogHeader>
            <DialogTitle>Save Range Scheme</DialogTitle>
            <DialogDescription>
              Save current ranges from {subnet?.prefix}/{subnet?.mask} as a
              reusable scheme.
            </DialogDescription>
          </DialogHeader>
          <form onSubmit={handleSaveRangeScheme} className="space-y-4 mt-2">
            <div className="space-y-1.5">
              ...
              <Input
                ...
                placeholder="e.g. Home Core Allocation"
                className="h-9 text-[13px]"
              />
            </div>
            <div className="space-y-1.5">
            ...
            <div className="rounded-md border border-border bg-(--surface-alt) px-3 py-2.5 text-xs text-(--text-muted)">
              This will save {subnet?.ipRanges.length || 0} range entries from
              the selected subnet.
            </div>
            <DialogFooter className="mt-6">
              <Button
                type="button"
                variant="outline"
                onClick={() => setSaveSchemeModalOpen(false)}
              >
                Cancel
              </Button>
      ...
      <Dialog
        open={applySchemeModalOpen && !!subnet}
        onOpenChange={setApplySchemeModalOpen}
      >
        <DialogContent>
          <DialogHeader>
            <DialogTitle>Apply Range Scheme</DialogTitle>
            <DialogDescription>
              Apply a saved multi-range allocation scheme to {subnet?.prefix}/
              {subnet?.mask}.
            </DialogDescription>
          </DialogHeader>
          <form onSubmit={handleApplyRangeScheme} className="space-y-4 mt-2">
              ...
              >
                ...
                {rangeSchemes.map((scheme) => (
                  <option key={scheme.id} value={scheme.id}>
                    {scheme.name} ({scheme.entries.length} entries)
                  </option>
                ))}
              </select>
            </div>
            <label className="flex items-center gap-2 text-[12px] font-medium text-(--text)">
              <input
                type="checkbox"
                checked={replaceExistingRanges}
                onChange={(e) => setReplaceExistingRanges(e.target.checked)}
              />
            ...
            <DialogFooter className="mt-6">
              <Button
                type="button"
                variant="outline"
                onClick={() => setApplySchemeModalOpen(false)}
              >
                Cancel
              </Button>
              <Button type="submit" disabled={!selectedSchemeId}>
                Apply Scheme
              </Button>
            </DialogFooter>
          </form>
      ...
      >
        <DialogContent>
          <DialogHeader>
            <DialogTitle>
              {editingIpId ? "Edit IP Address" : "Assign IP Address"}
            </DialogTitle>
          </DialogHeader>
          <form onSubmit={handleAssignIp} className="space-y-4 mt-2">
            <div className="space-y-1.5">
              ...
              >
                <option value="">
                  {editingIpId
                    ? "— Unassign device —"
                    : "No device — manual assignment"}
                </option>
                {devices.map((d) => (
                  <option key={d.id} value={d.id}>
                    {d.name} ({d.ipAddress || "no IP"}) — {d.category}
                  </option>
                ))}
              </select>
            </div>
            <div className="grid grid-cols-2 gap-5">
              <div className="space-y-1.5">
                ...
                <Input
                  ...
                  onChange={(e) =>
                    setIpForm({ ...ipForm, address: e.target.value })
                  }
                  placeholder="e.g. 10.0.10.20"
                  className="h-9 text-[13px]"
                />
              </div>
              <div className="space-y-1.5">
                ...
                <Input
                  value={ipForm.dnsName}
                  onChange={(e) =>
                    setIpForm({ ...ipForm, dnsName: e.target.value })
                  }
            ...
            <div className="space-y-1.5">
              ...
              <Input
                ...
                onChange={(e) =>
                  setIpForm({ ...ipForm, description: e.target.value })
                }
                placeholder="Optional"
                className="h-9 text-[13px]"
              />
            ...
            <DialogFooter className="mt-6 flex items-center">
              ...
              <Button type="submit">
                {editingIpId ? "Save Changes" : "Assign IP"}
              </Button>
            </DialogFooter>
          </form>
        </DialogContent>
        ...
        <DialogContent className="max-w-[400px] text-center">
          <DialogHeader className="flex flex-col items-center">
            ...
            <DialogDescription className="mt-2">
              This will remove the subnet and all associated IP addresses and
              ranges.
            </DialogDescription>
          </DialogHeader>
          <DialogFooter className="flex justify-center gap-4 sm:justify-center mt-4">
            <Button
              variant="outline"
              onClick={() => setDeleteSubnetModal(false)}
            >
              Cancel
            </Button>
            <Button variant="destructive" onClick={handleDeleteSubnet}>
              Delete Subnet
            </Button>
          </DialogFooter>
        </DialogContent>
