...
export default function SetupPage({ setupTokenRequired, onSetupComplete }: SetupPageProps) {
  ...
  const handleCreateOwner = async (e: FormEvent) => {
    ...
    try {
      ...
      const signInResult = await authClient.signIn.email({ email: email.trim().toLowerCase(), password })
      if (signInResult.error) {
        setError(signInResult.error.message || 'Owner created, but auto sign-in failed. Please sign in manually.')
        return
      }
    ...
    <div className="flex min-h-screen items-center justify-center p-4" style={{ background: 'var(--gradient-login-bg)' }}>
      <div className={cn(
        ...
        <form onSubmit={handleCreateOwner} className="space-y-4">
          <div>
            <Label htmlFor="owner-name" className="mb-2 block text-xs font-semibold text-muted-foreground">Display Name</Label>
            <Input
              id="owner-name"
              value={name}
          ...
          <div>
            <Label htmlFor="owner-email" className="mb-2 block text-xs font-semibold text-muted-foreground">Email</Label>
            <Input
              id="owner-email"
              type="email"
              value={email}
          ...
          <div>
            <Label htmlFor="owner-password" className="mb-2 block text-xs font-semibold text-muted-foreground">Password</Label>
            <Input
              id="owner-password"
              type="password"
              value={password}
          ...
          <div>
            <Label htmlFor="owner-password-confirm" className="mb-2 block text-xs font-semibold text-muted-foreground">Confirm Password</Label>
            <Input
