...
const WiFiView = ({
  ...
  const getSecurityLabel = (slug: string) =>
    securityOptionMap.get(slug)?.name || securityLabels[slug] || slug;
  const getSecurityColor = (slug: string) =>
    securityOptionMap.get(slug)?.color || securityColors[slug] || "#64748b";
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (res.ok) {
      ...
      alert("Failed to save WiFi network");
    }
  };
      ...
      if (selectedSecurityFilter) {
        if (
          securityOptions.some(
            (option) => option.slug === selectedSecurityFilter,
          )
        ) {
          if (n.security !== selectedSecurityFilter) return false;
        }
        if (selectedSecurityFilter === "wpa2" && !n.security.startsWith("wpa2"))
          return false;
        if (selectedSecurityFilter === "wpa3" && !n.security.startsWith("wpa3"))
          return false;
        if (selectedSecurityFilter === "open" && n.security !== "open")
          return false;
      }
      if (searchTerm) {
    ...
    <div className="animate-in fade-in duration-300">
      {networks.length > 0 && (
        <div className="flex justify-end mb-4">
          <Button size="sm" onClick={openCreate}>
            <Plus size={14} /> Create WiFi Network
          </Button>
        </div>
      )}
      ...
      <div className="grid grid-cols-4 gap-4 mb-6">
        {[
          ...
          {
            label: "Disabled",
            value: networks.length - enabledCount,
            color: "#94a3b8",
          },
        ].map((s, i) => (
          <div
      ...
      {networks.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-16 text-center bg-card border border-border rounded-lg">
          ...
          <p className="text-[13px] text-muted-foreground mb-6 max-w-[360px]">
            Create your first WiFi network to start managing wireless access.
          </p>
          ...
          <div className="grid grid-cols-[repeat(auto-fill,minmax(340px,1fr))] gap-4 mb-6">
            {filtered.map((n) => {
              ...
              return (
                <div
                  key={n.id}
                  className="bg-card border border-border rounded-[10px] p-5 relative"
                  style={{ opacity: n.enabled ? 1 : 0.6 }}
                >
                  <div className="flex justify-between items-start mb-4">
                    <div className="flex items-center gap-3">
                      <div
                        className="w-10 h-10 rounded-[10px] flex items-center justify-center"
                        style={{ background: `${secColor}14`, color: secColor }}
                      >
                        <Wifi size={20} />
                      </div>
                      <div>
                        ...
                        <div className="flex gap-1.5 items-center mt-0.5">
                          <span
                            className="inline-flex items-center gap-0.5 px-1.5 py-px rounded text-[9px] font-semibold"
                            style={{
                    ...
                    <div className="flex gap-0.5 items-center">
                      <Button
                        ...
                        size="icon"
                        className="h-7 w-7"
                        style={{ color: n.enabled ? "#10b981" : "#94a3b8" }}
                  ...
                  <div className="flex flex-wrap gap-1 mb-3">
                    <span
                      className={`px-1.5 py-px rounded text-[10px] font-semibold ${n.enabled ? "bg-(--green-bg) text-(--green)" : "bg-(--orange-bg) text-(--orange)"}`}
                    >
                      {n.enabled ? "Enabled" : "Disabled"}
                    </span>
                    {n.guestNetwork && (
                      <span className="inline-flex items-center gap-0.5 px-1.5 py-px rounded text-[10px] bg-(--orange-bg) text-(--orange)">
                        <Users size={8} /> Guest
                      </span>
                    )}
                    {n.hidden && (
                      <span className="inline-flex items-center gap-0.5 px-1.5 py-px rounded text-[10px] bg-(--muted-bg) text-(--text-slate)">
                        <EyeOff size={8} /> Hidden
                      </span>
                    )}
                    {n.clientIsolation && (
                      <span className="inline-flex items-center gap-0.5 px-1.5 py-px rounded text-[10px] bg-(--purple-bg) text-(--purple)">
                        <Shield size={8} /> Isolated
                      </span>
                    )}
                  </div>
                    ...
                    {n.subnet && (
                      <div className="flex justify-between">
                        ...
                        <code className="text-[11px] bg-(--muted-bg) px-1.5 py-px rounded">
                          {n.subnet.prefix}/{n.subnet.mask}
                        </code>
                      </div>
                    )}
                    {n.description && (
                      <div className="mt-1 italic text-[11px]">
                        {n.description}
                      </div>
                    )}
                  </div>
                  ...
                  <div className="mt-3 pt-3 border-t border-border flex gap-2 flex-wrap">
                    <span className="text-[10px] text-(--text-light) bg-(--surface-alt) px-1.5 py-0.5 rounded inline-flex items-center gap-0.5">
                      <Signal size={8} /> TX: {n.txPower}
                    </span>
            ...
            <table className="w-full border-collapse bg-(--surface) rounded-(--radius) border border-border table-fixed">
              ...
              <tbody>
                {filtered.map((n) => (
                  <tr
                    key={n.id}
                    data-highlight-id={n.id}
                    className={highlightId === n.id ? "highlight-flash" : ""}
                  ...
                  >
                    <td className="font-medium">
                      <div className="flex items-center gap-1.5">
                        <Wifi
                          size={13}
                          style={{ color: getSecurityColor(n.security) }}
                        />
                        {n.ssid}
                      </div>
                    </td>
                    <td>
                      <span
                        ...
                        style={{
                          background: `${getSecurityColor(n.security)}14`,
                          color: getSecurityColor(n.security),
                        }}
                      >
                        {getSecurityLabel(n.security)}
                      </span>
                    ...
                    <td>
                      <span
                        className={`px-2 py-0.5 rounded text-[11px] font-semibold ${n.enabled ? "bg-(--green-bg) text-(--green)" : "bg-(--orange-bg) text-(--orange)"}`}
                      >
                    ...
                    <td className="text-[11px] text-(--text-light)">
                      {[
                        n.guestNetwork && "Guest",
                        n.hidden && "Hidden",
                        n.clientIsolation && "Isolated",
                        n.bandSteering && "Band Steering",
                      ]
                        .filter(Boolean)
                        .join(", ") || "—"}
                    </td>
      ...
      >
        <DialogContent className="max-w-[560px]">
          <DialogHeader>
            <DialogTitle>
              {editingId ? "Edit WiFi Network" : "Create WiFi Network"}
            </DialogTitle>
          </DialogHeader>
          <form onSubmit={handleSubmit} className="space-y-4 mt-2">
            ...
            <div className="grid grid-cols-2 gap-5">
              <div>
                ...
                <select
                  ...
                  value={form.subnetId}
                  onChange={(e) =>
                    setForm({ ...form, subnetId: e.target.value })
                  }
                >
            ...
            <div className="grid grid-cols-3 gap-5 mt-3">
              <div>
                ...
                <select
                  className="w-full h-9 border border-border rounded bg-(--surface-alt) text-(--text) text-[13px] px-3 focus:outline-none focus:border-(--blue) focus:bg-(--surface)"
                  value={form.pmf}
                  onChange={(e) => setForm({ ...form, pmf: e.target.value })}
                >
              ...
              <div>
                ...
                <select
                  ...
                  value={form.txPower}
                  onChange={(e) =>
                    setForm({ ...form, txPower: e.target.value })
                  }
                >
              ...
              <div>
                ...
                <Input
                  ...
                  value={form.minRate}
                  onChange={(e) =>
                    setForm({ ...form, minRate: e.target.value })
                  }
                  placeholder="Auto"
                  min={1}
                  max={54}
            ...
            <DialogFooter className="mt-6">
              ...
              <Button type="submit">
                {editingId ? "Save Changes" : "Create Network"}
              </Button>
            </DialogFooter>
          </form>
        </DialogContent>
        ...
        <DialogContent className="max-w-[400px] text-center">
          <DialogHeader className="flex flex-col items-center">
            ...
            <DialogDescription className="mt-2">
              This will permanently remove this wireless network configuration.
            </DialogDescription>
          </DialogHeader>
