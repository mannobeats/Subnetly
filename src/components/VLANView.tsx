      ...
      {vlans.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-16 text-center bg-card border border-border rounded-lg">
          ...
          <p className="text-[13px] text-muted-foreground mb-6 max-w-[360px]">
            Create your first VLAN to start segmenting your network.
          </p>
          <Button onClick={openCreate}>
            <Plus size={14} /> Create VLAN
          </Button>
        </div>
      ) : (
        <>
          <div className="grid grid-cols-[repeat(auto-fill,minmax(320px,1fr))] gap-4 mb-6">
            {filteredVlans.map((v) => {
              ...
              const color =
                roleEntry?.color ||
                defaultRoleColors[v.role || ""] ||
                "#64748b";
              const Icon = roleEntry
                ? getCategoryIcon(roleEntry.icon)
                : Network;
              return (
                <div
                  key={v.id}
                  className="bg-(--surface) border border-border rounded-lg p-5 transition-shadow hover:shadow-md"
                >
                  ...
                  <div className="flex items-center gap-2 mb-3">
                    <span className="text-[11px] text-(--text-muted) font-semibold uppercase">
                      Role
                    </span>
                    <span
                  ...
                  {v.subnets.length > 0 && (
                    <div className="bg-(--surface-alt) border border-border rounded-md p-3 mb-3">
                      <span className="text-[10px] text-(--text-muted) font-semibold uppercase block mb-2">
                        Associated Subnets
                      </span>
                      {v.subnets.map((s) => (
                        <div
                        ...
                        >
                          <code className="text-xs font-medium bg-(--muted-bg) px-1.5 py-0.5 rounded">
                            {s.prefix}/{s.mask}
                          </code>
                          {s.gateway && (
                  ...
                  <div className="pt-2">
                    <div className="flex gap-2">
                      <span className="px-2 py-0.5 rounded text-[11px] font-semibold bg-(--blue-bg) text-(--blue)">
                        {v.subnets.length} subnet
                        {v.subnets.length !== 1 ? "s" : ""}
                      </span>
                      {v.role && (
                        <span
          ...
          <div className="bg-card border border-border rounded-lg overflow-hidden mt-6">
            ...
            <table className="w-full border-collapse table-fixed">
              ...
              <tbody>
                {filteredVlans.map((v) => {
                  const rc =
                    vlanRoles.find((r) => r.slug === v.role)?.color ||
                    defaultRoleColors[v.role || ""] ||
                    "#64748b";
                  return (
                    <tr
                      key={v.id}
                      data-highlight-id={v.id}
                      className={highlightId === v.id ? "highlight-flash" : ""}
                    >
                      <td>
                        <code
                      ...
                      <td>
                        <span
                          className={`px-2 py-0.5 rounded text-[11px] font-semibold ${v.status === "active" ? "bg-(--green-bg) text-(--green)" : "bg-(--orange-bg) text-(--orange)"}`}
                        >
                          {v.status}
                        </span>
                      </td>
                      <td>
                        {v.subnets.map((s) => (
                          <code
                            key={s.id}
                            className="text-[11px] bg-(--muted-bg) px-1.5 py-0.5 rounded mr-1"
                          >
                            {s.prefix}/{s.mask}
                          </code>
                        ))}
          ...
          <form onSubmit={handleSubmit} className="space-y-4 mt-2">
            ...
            <div className="grid grid-cols-2 gap-5">
              ...
              <div>
                ...
                >
                  {vlanStatuses.length > 0 ? (
                    vlanStatuses.map((status) => (
            ...
            <DialogFooter className="mt-6">
              ...
              <Button type="submit">
                {editingId ? "Save Changes" : "Create VLAN"}
              </Button>
            </DialogFooter>
          </form>
        </DialogContent>
        ...
        <DialogContent className="max-w-[400px] text-center">
          <DialogHeader className="flex flex-col items-center">
            ...
            <DialogDescription className="mt-2">
              This will remove the VLAN. Associated subnets will be unlinked.
            </DialogDescription>
          </DialogHeader>
